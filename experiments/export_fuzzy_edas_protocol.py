#!/usr/bin/env python3
"""Export fuzzy EDAS VISTA protocol data for R plotting.

Generates all VISTA results needed for a protocol run of fuzzy EDAS
and writes them to ``tmp/fuzzy_edas_protocol/`` as CSV files (via
``save_vista()``) plus a ``protocol_results.json`` summary.

Usage:
    python -m experiments.export_fuzzy_edas_protocol
    python -m experiments.export_fuzzy_edas_protocol --resolution 51
"""
...
from mcda_vista.fuzzy_utils import fuzzy_edas
...
OUTPUT_DIR = Path("tmp/fuzzy_edas_protocol")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Export fuzzy EDAS VISTA protocol data for R plotting.",
    )
    parser.add_argument(
        "--resolution",
        type=int,
