#!/usr/bin/env python3
"""Export fuzzy TOPSIS VISTA protocol data for R plotting.

Generates all VISTA results needed for a protocol run of fuzzy TOPSIS
and writes them to ``tmp/fuzzy_topsis_protocol/`` as CSV files (via
``save_vista()``) plus a ``protocol_results.json`` summary.

Usage:
    python -m experiments.export_fuzzy_topsis_protocol
    python -m experiments.export_fuzzy_topsis_protocol --resolution 51
"""
...
OUTPUT_DIR = Path("tmp/fuzzy_topsis_protocol")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Export fuzzy TOPSIS VISTA protocol data for R plotting.",
    )
    parser.add_argument(
        "--resolution",
        type=int,
